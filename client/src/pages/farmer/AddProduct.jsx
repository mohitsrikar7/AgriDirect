...
const AddProduct = ({ onAdded }) => {
  ...
  return (
    <div className="bg-white rounded-none shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
      {/* Header */}
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
        <div className="w-8 h-8 rounded-none bg-green-50 flex items-center justify-center">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-gray-500 font-medium mb-1.5">Product</label>
            <select name="masterProduct" value={form.masterProduct} required
              ...
              className="w-full border border-gray-200 rounded-none px-3 py-2.5 text-sm focus:ring-2 focus:ring-green-500/30 focus:border-green-500 outline-none transition-all appearance-none bg-white">
          ...
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-gray-500 font-medium mb-1.5">State (for price insight)</label>
        ...
        {/* Market Insight Card */}
        {recommendedPrice && (
          <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 border border-gray-200 rounded-none p-4">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-6 h-6 rounded-none bg-amber-100 flex items-center justify-center">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white rounded-none px-3 py-2">
                <p className="text-[10px] text-gray-500 font-medium">Mandi Price (quintal)</p>
                <p className="text-sm font-bold text-gray-900">₹{recommendedPrice.quintal}</p>
              </div>
              <div className="bg-white rounded-none px-3 py-2">
                <p className="text-[10px] text-gray-500 font-medium">Suggested / kg</p>
                <p className="text-sm font-bold text-green-700">₹{recommendedPrice.perKg}</p>
              </div>
              <div className="bg-white rounded-none px-3 py-2">
                <p className="text-[10px] text-gray-500 font-medium">Recommended Range</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-gray-500 font-medium mb-1.5">Total Value (₹)</label>
            <input name="price" type="number" placeholder="e.g. 5000" value={form.price} onChange={handleChange} required
              className="w-full border border-gray-200 rounded-none px-4 py-2.5 text-sm focus:ring-2 focus:ring-green-500/30 focus:border-green-500 outline-none transition-all" />
          </div>
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-gray-500 font-medium mb-1.5">Quantity (kg)</label>
        ...
        {/* Price Status Badge */}
        {priceStatus && (
          <div className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-none border ${priceStatus.color}`}>
        ...
        {form.price && form.quantity && Number(form.quantity) > 0 && (
          <div className="bg-gray-50 border border-gray-100 rounded-none p-4">
            <p className="text-[10px] uppercase tracking-wider text-gray-500 font-medium mb-2">Revenue Preview</p>
        ...
        <button type="submit" disabled={submitting}
          className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 rounded-none text-sm font-semibold shadow-lg shadow-green-600/25 transition-all">
          {submitting ? (
