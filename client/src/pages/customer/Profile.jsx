...
const Profile = () => {
    ...
    <div className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-4">
        {[
          ...
          <div key={key}>
            <label className="block text-[10px] font-bold uppercase tracking-[0.15em] text-brand-muted mb-1.5">{placeholder}</label>
            <input
              placeholder={placeholder}
              value={form[key] || ""}
              onChange={(e) => setForm({ ...form, [key]: e.target.value })}
              className="w-full px-4 py-3 bg-surface border border-border rounded-lg text-sm text-brand
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button
          onClick={onCancel}
          className="text-brand-muted hover:text-brand px-4 py-2.5 text-sm font-medium transition-colors"
        >
          Cancel
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <div className="bg-surface-light pt-28 pb-24 px-6 border-b border-border">
        <div className="max-w-3xl mx-auto flex items-center gap-5">
          <div className="w-20 h-20 bg-brand border border-brand rounded-2xl flex items-center justify-center flex-shrink-0">
            <span className="text-3xl font-bold text-white">{user?.name?.charAt(0).toUpperCase()}</span>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-[0.2em] text-brand-muted font-bold mb-1">Account</p>
            <h1 className="text-3xl md:text-4xl font-black text-brand tracking-tight">{user?.name}</h1>
            <p className="text-brand-muted text-sm mt-1">{user?.email}</p>
            <div className="flex items-center gap-2 mt-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-brand-muted bg-surface border border-border px-3 py-1.5 rounded-full">
                Member since {user?.createdAt ? new Date(user.createdAt).toLocaleDateString("en-IN", { month: "short", year: "numeric" }) : "N/A"}
              </span>
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-accent bg-accent-muted px-3 py-1.5 rounded-full flex items-center gap-1">
      ...
      <div className="max-w-3xl mx-auto px-6 py-12 space-y-8">
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center gap-2">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-brand-muted">Profile Information</h2>
          </div>
          <div className="p-6">
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-[0.15em] text-brand-muted mb-1.5">Full Name</label>
                <input
                  value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full bg-surface border border-border rounded-lg px-4 py-3 text-sm text-brand
                    focus:ring-2 focus:ring-accent/20 focus:border-accent outline-none transition-all"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-[0.15em] text-brand-muted mb-1.5">Phone</label>
                <input
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center gap-2">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-brand-muted">Delivery Addresses</h2>
          </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] font-bold text-brand bg-surface border border-border px-2.5 py-1 rounded-full uppercase tracking-[0.15em]">Primary</span>
              </div>
              ...
              {!primaryAddress && !editingPrimary && (
                <button
                  ...
                  className="w-full border-2 border-dashed border-border hover:border-accent rounded-lg py-6 text-sm text-brand-muted hover:text-accent font-medium transition-all flex items-center justify-center gap-2"
                >
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[10px] font-bold text-brand-muted bg-surface border border-border px-2.5 py-1 rounded-full uppercase tracking-[0.15em]">Secondary</span>
              </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <button
            ...
            className="group bg-white rounded-xl border border-border p-5 text-left hover:border-brand transition-all duration-300"
          >
            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-muted mb-2">Orders</h3>
            <p className="font-bold text-brand text-sm">View My Orders</p>
            <p className="text-xs text-brand-muted mt-1 font-light">Track and manage your orders</p>
            <div className="mt-3 flex items-center gap-1 text-xs font-bold text-brand group-hover:gap-2 transition-all uppercase tracking-wider">
              View →
            </div>
          </button>
          ...
          <button
            ...
            className="group bg-white rounded-xl border border-border p-5 text-left hover:border-brand transition-all duration-300"
          >
            <h3 className="text-[10px] font-bold uppercase tracking-[0.2em] text-brand-muted mb-2">Marketplace</h3>
            <p className="font-bold text-brand text-sm">Browse Products</p>
            <p className="text-xs text-brand-muted mt-1 font-light">Discover fresh farm produce</p>
            <div className="mt-3 flex items-center gap-1 text-xs font-bold text-brand group-hover:gap-2 transition-all uppercase tracking-wider">
              Explore →
            </div>
          </button>
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="px-6 py-4 border-b border-border flex items-center gap-2">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-brand-muted">Security</h2>
          </div>
          <div className="p-6">
            {!showPasswordForm ? (
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold text-brand">Password</p>
                  <p className="text-xs text-brand-muted mt-0.5 font-light">Last changed: Unknown</p>
                </div>
                <button
        ...
        <div className="text-center pt-2">
          <p className="text-[11px] text-brand-muted flex items-center justify-center gap-1.5 uppercase tracking-wider font-medium">
